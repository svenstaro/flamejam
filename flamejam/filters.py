...
def _delta(delta, short=True):
    ...
    if delta.years > 0:
        return (_s(delta.years, "year") +
                ("" if short and delta.years > threshold else
                 (" " + _s(delta.months, "month"))))
    if delta.months > 0:
        return (_s(delta.months, "month") +
                ("" if short and delta.months > threshold else
                 (" " + _s(delta.days, "day"))))
    if delta.days > 0:
        return (_s(delta.days, "day") +
                ("" if short and delta.days > threshold else
                 (" " + _s(delta.hours, "hour"))))
    if delta.hours > 0:
        return (_s(delta.hours, "hour") +
                ("" if short and delta.hours > threshold else
                 (" " + _s(delta.minutes, "minute"))))
    if delta.minutes > 0:
        return (_s(delta.minutes, "minute") +
                ("" if short and delta.minutes > threshold else
                 (" " + _s(delta.seconds, "second"))))
...
def humandelta(s, other=None, short=True):
    ...
    if (s.seconds < 0 or s.minutes < 0 or s.hours < 0 or
            s.days < 0 or s.months < 0 or s.years < 0):
        delta = _delta(-s, short).strip()
        return f"{delta} ago"
    elif (s.seconds > 0 or s.minutes > 0 or s.hours > 0 or
            s.days > 0 or s.months > 0 or s.years > 0):
        delta = _delta(s, short).strip()
        return f"in {delta}"
    else:
...
def humantime(s, short=True):
    ...
    if diff.months < 1:
        markup = f'<span title="{formattime(s)}" class="time-title">'
        markup += f'{humandelta(diff, short=short)}</span>'
