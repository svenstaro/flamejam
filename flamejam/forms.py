...
class Not(object):
    ...
    def __call__(self, form, field):
        ...
        if not errored:
            raise ValidationError(self.call.message if self.message is None else self.message)
...
class EmailExists(object):
    def __call__(self, form, field):
        e = User.query.filter(
            func.lower(User.email) == func.lower(field.data)).first()
        if not e:
            raise ValidationError("That email does not exist")


class LoginValidator(object):
    def __init__(self, pw_field, message_username="The username or password is incorrect.",
...
class UserRegistration(Form):
    username = TextField("Username", validators=[
        Not(MatchesRegex("[^0-9a-zA-Z\-_]"), message=(
            "Your username contains invalid characters. Only use alphanumeric characters, "
            "dashes and underscores.")),
        Not(UsernameExists(), message="That username already exists."),
        Length(min=3, max=80, message=(
            "You have to enter a username of 3 to 80 characters length.")),
    ])
    password = PasswordField("Password", validators=[
        Length(min=8, message="Please enter a password of at least 8 characters."),
    ])
...
class JamDetailsForm(Form):
    ...
    registration_duration = IntegerField("Registration duration", validators=[
        Required(),
        NumberRange(min=0),
    ], default=14 * 24)
    packaging_duration = IntegerField("Packaging duration", validators=[
        Required(),
        NumberRange(min=0),
    ], default=24)
    rating_duration = IntegerField("Rating duration", validators=[
        Required(),
        NumberRange(min=0),
    ], default=24 * 5)
...
class GameAddPackageForm(Form):
    ...
    type = SelectField("Type", choices=[
        ("web",          GamePackage.type_string("web")),
        ("linux",        GamePackage.type_string("linux")),
        ("linux32",      GamePackage.type_string("linux32")),
        ("linux64",      GamePackage.type_string("linux64")),
        ("windows",      GamePackage.type_string("windows")),
        ("windows64",    GamePackage.type_string("windows64")),
        ("mac",          GamePackage.type_string("mac")),
        ("source",       GamePackage.type_string("source")),
        ("git",          GamePackage.type_string("git")),
        ("svn",          GamePackage.type_string("svn")),
        ("hg",           GamePackage.type_string("hg")),
        ("combi",        GamePackage.type_string("combi")),
        ("love",         GamePackage.type_string("love")),
        ("blender",      GamePackage.type_string("blender")),
        ("unknown",      GamePackage.type_string("unknown"))])


class RateGameForm(Form):
    score = IntegerField("Overall rating", validators=[
        Required(),
...
for x in RATING_CATEGORIES:
    setattr(RateGameForm, f"score_{x}",
            IntegerField(f"{x.capitalize()} rating",
...
class SettingsForm(Form):
    ...
    new_password2 = PasswordField("New Password, again", validators=[
...
class CancelParticipationForm(Form):
    confirm = BooleanField("I understand that, please cancel my participation",
                           validators=[Required()])


class LeaveTeamForm(Form):
    confirm = BooleanField("I understand that, and want to leave the team",
                           validators=[Required()])
...
class AdminUserForm(Form):
    ...
    avatar = TextField("Avatar URL", validators=[
        Optional(),
        URL(),
    ])
    email = EmailField("Email", validators=[
